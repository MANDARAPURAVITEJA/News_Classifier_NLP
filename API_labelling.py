...
dictionary='https://api.gdeltproject.org/api/v2/context/context?format=html&timespan=15H&query=india&mode=artlist&maxrecords=75&format=json&sort=DateDesc#'
...
def db2CSV():
    ...
    df = pd.read_sql_query(qry, db)
    df.to_csv("DB.csv")
    return "CSV file updated with name DB.csv"
#%%
